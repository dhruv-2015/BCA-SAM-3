...
create view capitalCitieswithPopulationGreaterThan10Million as
select * from country where population > 10000000
select * from capitalCitieswithPopulationGreaterThan10Million
-- 4. Create a view that displays the state with the highest population for each country, along with its population figure.
create view MX_Population as
select country.country_name, state.state_name, state.population from country inner join state on country.country_id = state.country_id where state.population = (select max(population) from state where country_id = country.country_id )
-- Part – C
-- 5. Create a view that lists states that do not have a designated capital.
create view statesThatDoNotHaveDesignatedCapital as
select * from state where capital is null
-- 6. Create a view that displays countries with more than one capital city
create view moreThanOneCapital as
select country_name from country group by country_name having COUNT(capital) > 1